...
export default function ProjectPinsPage() {
  ...
  const totalCost = useMemo(
    () => pins.reduce((sum, p) => sum + p.total_cost, 0),
    [pins]
  );
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-5 py-8">
        ...
        <div className="panel mb-5 flex flex-wrap items-center gap-3 p-4">
          <select
            ...
            onChange={(e) =>
              setStatusFilter(
                e.target.value as PinStatus | "all"
              )
            }
          >
          ...
          <span className="label-mono ml-auto">
            {pins.length} pin
            {pins.length === 1 ? "" : "s"} ·{" "}
            {formatPrice(totalCost)} in materials
          </span>
        </div>
        ...
        {fetching ? (
          <div className="flex flex-col gap-2">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className="panel h-12 animate-pulse"
                style={{ opacity: 0.5 }}
              />
            ))}
          </div>
          ...
          <div
            className="panel px-5 py-10 text-center text-sm"
            style={{ color: "var(--paper-dim)" }}
          >
                  ...
                  {pins.map((p) => (
                    ...
                    >
                      <td className="px-3 py-3">
                        <Link
                          href={`/projects/${projectId}/sheets/${p.sheet_id}?pin=${p.id}`}
